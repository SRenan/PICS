...
http:/wiki.rglab.org/index.php?title=Public:Mappability\_Profile
...
dataIP<-read.table(file.path(datadir, "Treatment_tags_chr21_sort.bed"), header=TRUE)
dataIP<-as(dataIP,"GRanges")

dataCont<-read.table(file.path(datadir, "Input_tags_chr21_sort.bed"), header=TRUE)
dataCont<-as(dataCont,"GRanges")
@

<<Reading the mappability profile>>=
map <- read.table(file.path(datadir, "mapProfileShort"), header=TRUE)
map <- as(map, "GRanges")
@
...
<<plot-FDR1>>=
plot(pics, picsC, xlim = c(2, 8), ylim = c(0, .2),
  filter = list(delta = c(50, 300), se = c(0, 50), sigmaSqF = c(0, 22500),
  sigmaSqR = c(0, 22500)), type = "l")
@
...
\section{Output}
Any \texttt{picsList} object can be coerced into a \texttt{data.frame} for 
visualization and manipulation of the results.
<<as.data.frame>>=
df <- as.data.frame(pics)
head(df)
@
