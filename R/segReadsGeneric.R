...
#' @importFrom GenomicRanges seqnames strand start end
#' @importMethodsFrom GenomicRanges [ [[ [<- [[<-
#' @export
segReadsGeneric <- function(data, dataC = NULL, map = NULL, minReads = 2,
                            minReadsInRegion = 3, jitter = FALSE, maxLregion = 0,
                            minLregion = 100, step = 20, width = 250, package = "PICS"){
  ...
  if(is.null(minReads)){
    ...
    print(paste0("We automatically calculated minReads, which is ", minReads,"."))
  }
  
  minReadsInRegion=max(minReadsInRegion,minReads)
  
  paraSW <- list(step = as.integer(step), width = as.integer(width),
  ...
  if (maxLregion>0){
    maxStep <- (maxLregion-2*paraSW$width)/paraSW$step
  } else{
    maxStep <- 0
  }
  ...
  newSegReadsList<-.Call("segReadsAll", data, dataC, start, end,
                         as.integer(jitter), paraSW , as.integer(maxStep),
                         as.integer(minLregion), pPackage=package, PACKAGE="PICS")
...
#' 
#' Returns info about a segReadsList object in a \code{data.frame} containing
#' the following informations: chr : chromosome id NF : number of forward reads
#' NR : number of reverse reads L : length of segment min: start location of
#' segments max: end location of segments
#' 
#' 
#' @param seg An object of class \code{segReadsList}
#' @return A six columns \code{data.frame}.
#' @author Xuekui Zhang
...
summarySeg <- function(seg){
  ...
  ans <- data.frame(chr = temp[[1]], NF = temp[[2]], NR = temp[[3]],
                    L = temp[[4]], min = temp[[5]], max = temp[[6]])
